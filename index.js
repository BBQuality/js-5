...
function sumArray(array) {
  ...
  let sum = 0;
       ...
       for (const index in array) {
          ...
          for (const pointIndex in array[index]) {
...
function printUppercase(array) {
  ...
  for (const index of array){
    ...
    array[index]=index.toUpperCase(array); 
...
const array = [firstElement, ...restElements];

minArg = array[firstElement];

maxArg = array[firstElement];
console.log(array); 
...
function concatenateStrings(array, subString) {
  ...
  return list.includes(sub) ? true : false;
...
function multiplyByIndex(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
      result[i] = arr[i] * i; // Присвоюємо значення під індексом `i` у масиві `result`
    }
